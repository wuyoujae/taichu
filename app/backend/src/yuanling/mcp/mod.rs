...
mod tests {
  ...
  fn test_dir(name: &str) -> PathBuf {
    let millis = SystemTime::now()
      ...
      .as_millis();
    let path = env::temp_dir().join(format!("taichu-mcp-{name}-{millis}"));
    fs::create_dir_all(&path).expect("test dir should be created");
    path
  }

  fn write_fake_mcp_server() -> PathBuf {
    let root = test_dir("server");
    let script = root.join("fake_mcp_server.py");
    fs::write(
      &script,
      r#"import json
import sys
...
"#,
  ...
  fn fake_config(script: &Path) -> McpModuleConfig {
    McpModuleConfig {
      enabled: true,
      ...
      list_tools_timeout_ms: 1_000,
      default_tool_call_timeout_ms: 1_000,
      resource_timeout_ms: 1_000,
    }
  }

  #[test]
  fn normalizes_mcp_tool_names() {
    assert_eq!(normalize_name_for_mcp("github.com"), "github_com");
    assert_eq!(mcp_tool_name("my server", "echo text"), "mcp__my_server__echo_text");
  }

  #[test]
  fn parses_servers_json() {
    let servers = parse_servers_json(
      r#"{"alpha":{"type":"stdio","command":"python3","args":["server.py"],"env":{"A":"B"},"tool_call_timeout_ms":42}}"#,
    )
    .expect("servers should parse");
    assert_eq!(servers.len(), 1);
    assert_eq!(servers["alpha"].transport(), McpTransportKind::Stdio);
  }

  #[test]
  fn content_length_frame_is_encoded() {
    let frame = encode_frame(br#"{"jsonrpc":"2.0"}"#);
    assert!(frame.starts_with(b"Content-Length: 17\r\n\r\n"));
  }

  #[test]
  fn preflight_reports_missing_and_unsupported_servers() {
    let config = McpModuleConfig {
      enabled: true,
      servers: BTreeMap::from([
        (
          "missing".to_string(),
          McpServerConfig::Stdio {
            command: "definitely-not-a-real-taichu-command".to_string(),
            args: Vec::new(),
            env: BTreeMap::new(),
      ...
      list_tools_timeout_ms: 1,
      default_tool_call_timeout_ms: 1,
      resource_timeout_ms: 1,
    };
    let report = preflight_config(&config);

    assert!(report.degraded);
    assert!(report
      .servers
      .iter()
      .any(|server| server.status == McpServerStatus::CommandNotFound));
    assert!(report
      .servers
      .iter()
      .any(|server| server.status == McpServerStatus::UnsupportedTransport));
  }

  #[test]
  fn manager_discovers_calls_and_reads_stdio_server() {
    let runtime = tokio::runtime::Builder::new_current_thread()
      .enable_all()
      .build()
      .expect("runtime should build");
    runtime.block_on(async {
      let script = write_fake_mcp_server();
      let config = fake_config(&script);
      let mut manager = McpServerManager::from_config(&config);

      let tools = manager.discover_tools().await.expect("tools should discover");
      ...
      assert_eq!(call.structured_content, Some(json!({"echoed": "hello"})));
      assert_eq!(call.content[0].data.get("text"), Some(&json!("echo:hello")));
      ...
      manager.shutdown().await.expect("manager should shutdown");
    });
  }

  #[test]
  fn runtime_tool_definitions_use_qualified_names() {
    let managed = vec![ManagedMcpTool {
      server_name: "alpha".to_string(),
      raw_name: "echo".to_string(),
      qualified_name: mcp_tool_name("alpha", "echo"),
      tool: McpTool {
        name: "echo".to_string(),
    ...
    let definitions = runtime_tool_definitions(&managed);

    assert_eq!(definitions[0].name, "mcp__alpha__echo");
    assert_eq!(definitions[0].required_permission, ToolPermissionMode::DangerFullAccess);
  }

  #[test]
  fn mcp_tool_executor_calls_discovered_tool() {
    let script = write_fake_mcp_server();
    let config = fake_config(&script);
    let mut executor = McpToolExecutor::new(config);
    let output = executor
      .execute("mcp__alpha__echo", &json!({"text": "executor"}))
      .expect("mcp executor should call tool");

    assert_eq!(output.tool_name, "mcp__alpha__echo");
    assert_eq!(output.output["structuredContent"], json!({"echoed": "executor"}));
  }
}
