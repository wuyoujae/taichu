...
#[cfg(test)]
mod tests {
  use crate::yuanling::contact::{load_contact, set_contact_status, ContactModuleConfig, ContactStatus};
  use super::{
    BuiltinToolExecutor, RuntimeToolDefinition, StaticToolExecutor, ToolError, ToolExecutor,
    ToolPermissionDecision, ToolPermissionMode, ToolPermissionOutcome, ToolPermissionPrompter,
    ToolPermissionRequest, ToolRegistry, ToolsModuleConfig, ToolFilesystemScope,
  };
  use serde_json::{json, Value};
  ...
  struct TestPrompter {
    ...
    requests: Vec<ToolPermissionRequest>,
  }
  ...
  impl ToolPermissionPrompter for TestPrompter {
    fn confirm(&mut self, request: &ToolPermissionRequest) -> ToolPermissionDecision {
  ...
  fn default_config() -> ToolsModuleConfig {
    ToolsModuleConfig {
      enabled: true,
      allowed_tools: None,
  ...
  fn test_workspace(name: &str) -> PathBuf {
    let millis = SystemTime::now()
      ...
      .as_millis();
    let root = std::env::temp_dir().join(format!("taichu-tools-{name}-{millis}"));
    fs::create_dir_all(&root).expect("workspace should be created");
    root
  }

  fn contact_env_lock() -> &'static Mutex<()> {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| Mutex::new(()))
  }

  #[test]
  fn builtin_registry_exposes_core_tool_definitions() {
    let registry = ToolRegistry::builtin();
    ...
    assert!(names.contains(&"McpAuth".to_string()));
    assert!(names.contains(&"Sleep".to_string()));
    assert!(names.contains(&"REPL".to_string()));
    assert!(names.contains(&"PowerShell".to_string()));
  }

  #[test]
  fn send_message_tool_schema_and_permission_are_read_only() {
    let registry = ToolRegistry::builtin();
    let view = registry
      .views(None)
      .into_iter()
      .find(|tool| tool.name == "send_message")
      .expect("send_message should be registered");

    assert_eq!(view.required_permission, 1);
    assert_eq!(view.permission_label, "read_only");
    assert_eq!(
      view.input_schema["required"],
      json!(["from_yuanling_id", "to_yuanling_id", "content"])
    );
    assert_eq!(view.input_schema["additionalProperties"], Value::Bool(false));
  }

  #[test]
  fn runtime_tool_conflicting_with_builtin_fails() {
    let result = ToolRegistry::builtin().with_runtime_tools(vec![RuntimeToolDefinition {
      name: "read_file".to_string(),
      description: None,
  ...
  #[test]
  fn duplicate_runtime_tool_fails() {
    let result = ToolRegistry::builtin().with_runtime_tools(vec![
      RuntimeToolDefinition {
        name: "runtime_one".to_string(),
        description: None,
        input_schema: json!({"type": "object"}),
        required_permission: ToolPermissionMode::ReadOnly,
      },
  ...
  #[test]
  fn allowed_tools_support_aliases_and_separators() {
    let registry = ToolRegistry::builtin();
    let allowed = registry
      .normalize_allowed_tools(&["read, write grep".to_string()])
      .expect("allowed should normalize")
      .expect("allowed should exist");
  ...
  #[test]
  fn definitions_filter_by_allowed_tools() {
    let registry = ToolRegistry::builtin();
    let allowed = registry
      .normalize_allowed_tools(&["read".to_string()])
      .expect("allowed should normalize");
    let definitions = registry.definitions(allowed.as_ref());
  ...
  #[test]
  fn search_matches_name_and_description_with_limit() {
    let registry = ToolRegistry::builtin();
    let output = registry.search("file", 2);

    assert_eq!(output.matches.len(), 2);
    assert!(output.matches.iter().any(|item| item.name == "read_file"));
  }

  #[test]
  fn tool_search_executes_as_registry_builtin() {
    let registry = ToolRegistry::builtin();
    let mut executor = StaticToolExecutor::new();
    let output = registry
      .execute(
        "ToolSearch",
        &json!({"query": "grep", "max_results": 5}),
        None,
        &mut executor,
      )
      .expect("ToolSearch should execute");

    assert_eq!(output.tool_name, "ToolSearch");
    assert_eq!(output.output["matches"][0]["name"], Value::String("grep_search".to_string()));
  }

  #[test]
  fn read_only_permission_is_auto_allowed_by_default() {
    let registry = ToolRegistry::builtin();
    ...
    assert_eq!(output.permission_outcome, Some(ToolPermissionOutcome::AutoAllowed));
    assert_eq!(output.output["ok"], Value::Bool(true));
  }

  #[test]
  fn send_message_permission_is_auto_allowed_as_read_only_tool() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut executor =
      StaticToolExecutor::new().register("send_message", |_| Ok(json!({"status": "queued"})));
    let output = registry
      .execute_with_permissions(
        "send_message",
        &json!({
          "from_yuanling_id": "sender",
          "to_yuanling_id": "receiver",
          "content": "hello"
        }),
        &config,
        &mut executor,
      ...
      .expect("send_message should execute without prompt");

    assert_eq!(output.permission_outcome, Some(ToolPermissionOutcome::AutoAllowed));
    assert_eq!(output.output["status"], Value::String("queued".to_string()));
  }

  #[test]
  fn workspace_write_permission_uses_prompter_and_allows_execution() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut prompter = TestPrompter::allow();
    let mut executor = StaticToolExecutor::new().register("write_file", |_| Ok(json!({"ok": true})));
    let output = registry
      .execute_with_permissions(
        "write_file",
      ...
      .expect("write tool should execute after approval");

    assert_eq!(output.permission_outcome, Some(ToolPermissionOutcome::UserAllowed));
    assert_eq!(prompter.requests.len(), 1);
    assert_eq!(prompter.requests[0].required_permission, 2);
  }

  #[test]
  fn danger_permission_uses_prompter_and_allows_execution() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut prompter = TestPrompter::allow();
    let mut executor =
      StaticToolExecutor::new().register("bash", |_| Ok(json!({"status": "ok"})));
    let output = registry
      .execute_with_permissions(
        "bash",
      ...
      .expect("danger tool should execute after approval");

    assert_eq!(output.permission_outcome, Some(ToolPermissionOutcome::UserAllowed));
    assert_eq!(prompter.requests[0].required_permission, 3);
  }

  #[test]
  fn denied_permission_does_not_execute_tool() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut prompter = TestPrompter::deny("not now");
    let mut executor = StaticToolExecutor::new();
    let result = registry.execute_with_permissions(
      "write_file",
      &json!({"path": "tmp.txt", "content": "hello"}),
      &config,
      &mut executor,
  ...
  #[test]
  fn missing_prompter_blocks_tools_that_need_confirmation() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut executor = StaticToolExecutor::new();
    let result = registry.execute_with_permissions(
    ...
    assert!(matches!(result, Err(ToolError::PermissionRequired(_))));
  }

  #[test]
  fn disabled_tools_module_blocks_before_execution() {
    let registry = ToolRegistry::builtin();
    ...
    assert!(matches!(result, Err(ToolError::ModuleDisabled)));
  }

  #[test]
  fn allowed_tools_filter_runs_before_permission_prompt() {
    let registry = ToolRegistry::builtin();
    let mut config = default_config();
    config.allowed_tools = Some(BTreeSet::from(["read_file".to_string()]));
    let mut prompter = TestPrompter::allow();
    let mut executor = StaticToolExecutor::new();
    let result = registry.execute_with_permissions(
      "write_file",
    ...
    assert!(matches!(result, Err(ToolError::NotAllowed(_))));
    assert!(prompter.requests.is_empty());
  }

  #[test]
  fn tool_search_is_auto_allowed_as_read_only_tool() {
    let registry = ToolRegistry::builtin();
    let config = default_config();
    let mut executor = StaticToolExecutor::new();
    let output = registry
      ...
      .expect("ToolSearch should execute without prompt");
  ...
  #[test]
  fn builtin_executor_runs_file_search_sleep_and_repl_tools() {
    let workspace = test_workspace("builtin");
    let registry = ToolRegistry::builtin();
    let mut config = default_config();
    config.permission_policy.confirm_workspace_write = false;
    config.permission_policy.confirm_danger_full_access = false;
    let mut executor = BuiltinToolExecutor::new(&workspace).expect("executor should build");

    registry
      .execute_with_permissions(
        "write_file",
        &json!({"path": "notes/a.txt", "content": "hello world\nsecond line"}),
        &config,
        &mut executor,
        None,
      )
    ...
    let read = registry
      .execute_with_permissions(
        "read_file",
        &json!({"path": "notes/a.txt", "offset": 0, "limit": 1}),
        &config,
        &mut executor,
        None,
      )
      .expect("read should work");
    assert_eq!(read.output["content"], Value::String("hello world".to_string()));
    ...
    let glob = registry
      .execute_with_permissions(
        "glob",
        &json!({"pattern": "*.txt", "path": "notes"}),
        &config,
        &mut executor,
        None,
      )
      .expect("glob should work");
    assert_eq!(glob.output["matches"][0], Value::String("notes/a.txt".to_string()));

    let grep = registry
      .execute_with_permissions(
        "grep",
        &json!({"pattern": "HELLO", "path": "notes", "include": "*.txt"}),
        &config,
        &mut executor,
        None,
      )
      .expect("grep should work");
    assert_eq!(grep.output["matches"][0]["line_number"], Value::from(1));
    ...
    assert_eq!(sleep.output["requested_ms"], Value::from(1));

    let repl = registry
      .execute_with_permissions(
        "REPL",
        &json!({"language": "bash", "code": "echo repl-ok", "timeout_ms": 1000}),
        &config,
        &mut executor,
        None,
      )
      .expect("repl should work");
    assert_eq!(repl.output["success"], Value::Bool(true));
    assert!(repl.output["stdout"].as_str().unwrap_or("").contains("repl-ok"));
  }

  #[test]
  fn builtin_executor_runs_send_message_and_persists_receiver_contact() {
    let _guard = contact_env_lock().lock().expect("contact env lock should work");
    let workspace = test_workspace("send-message");
    let contact_storage = workspace.join("contact");
    std::env::set_var("YUANLING_CONTACT_ENABLED", "true");
    std::env::set_var(
      "YUANLING_CONTACT_STORAGE_DIR",
      contact_storage.to_string_lossy().to_string(),
    );
    ...
    let output = registry
      .execute_with_permissions(
        "send_message",
        &json!({
          "from_yuanling_id": "sender-tool",
          "to_yuanling_id": "receiver-tool",
      ...
      .expect("send_message should execute");

    let contact_config = ContactModuleConfig {
      enabled: true,
      storage_dir: contact_storage.to_string_lossy().to_string(),
    };
    let receiver = load_contact("receiver-tool", &contact_config).expect("contact should load");

    assert_eq!(output.output["status"], Value::String("queued".to_string()));
    assert_eq!(receiver.pending_messages.len(), 1);
    assert_eq!(receiver.pending_messages[0].from_yuanling_id, "sender-tool");
    assert_eq!(receiver.pending_messages[0].content, "hello from tool");
  }

  #[test]
  fn builtin_executor_reports_disabled_contact_target() {
    let _guard = contact_env_lock().lock().expect("contact env lock should work");
    let workspace = test_workspace("send-message-disabled");
    let contact_storage = workspace.join("contact");
    std::env::set_var("YUANLING_CONTACT_ENABLED", "true");
    std::env::set_var(
      "YUANLING_CONTACT_STORAGE_DIR",
    ...
    set_contact_status("receiver-disabled", ContactStatus::Disabled, &contact_config)
    ...
    let result = registry.execute_with_permissions(
      ...
      &json!({
        "from_yuanling_id": "sender-tool",
        "to_yuanling_id": "receiver-disabled",
        "content": "hello"
      }),
      &config,
      &mut executor,
  ...
  #[test]
  fn builtin_executor_rejects_path_traversal() {
    let workspace = test_workspace("traversal");
    let mut executor = BuiltinToolExecutor::new(&workspace).expect("executor should build");
    let result = executor.execute("read_file", &json!({"path": "../Cargo.toml"}));

    assert!(matches!(result, Err(ToolError::InvalidInput(_))));
  }

  #[test]
  fn execute_reports_unknown_not_allowed_and_missing_handler() {
    let registry = ToolRegistry::builtin();
    let mut executor = StaticToolExecutor::new();
    let allowed = registry
      .normalize_allowed_tools(&["read".to_string()])
    ...
    let unknown = registry.execute("missing", &json!({}), None, &mut executor);
    assert!(matches!(unknown, Err(ToolError::UnknownTool(_))));

    let not_allowed = registry.execute("write_file", &json!({}), allowed.as_ref(), &mut executor);
    assert!(matches!(not_allowed, Err(ToolError::NotAllowed(_))));

    let missing_handler = registry.execute("read_file", &json!({}), None, &mut executor);
    assert!(matches!(missing_handler, Err(ToolError::MissingHandler(_))));
  }

  #[test]
  fn static_tool_executor_executes_registered_handler() {
    let mut executor = StaticToolExecutor::new().register("read_file", |input| {
      Ok(json!({
        "path": input.get("path").and_then(Value::as_str).unwrap_or(""),
        "content": "ok"
      }))
    });
    let output = executor
    ...
    assert_eq!(output.output["content"], Value::String("ok".to_string()));
  }
}
