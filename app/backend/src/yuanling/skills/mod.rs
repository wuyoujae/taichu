...
mod tests {
  use super::{
    install_skill, SkillError, SkillOrigin, SkillRegistry, SkillRoot, SkillSource,
    SkillsModuleConfig,
  };
  ...
  fn test_dir(name: &str) -> PathBuf {
    let millis = SystemTime::now()
      ...
      .as_millis();
    let path = std::env::temp_dir().join(format!("taichu-skills-{name}-{millis}"));
    fs::create_dir_all(&path).expect("test dir should be created");
    path
  }

  fn write_skill(root: &PathBuf, id: &str, name: &str, description: &str) {
  ...
  fn config(root: PathBuf) -> SkillsModuleConfig {
    SkillsModuleConfig {
      enabled: true,
      roots: vec![SkillRoot {
        source: SkillSource::DataDir,
        path: root,
        origin: SkillOrigin::SkillsDir,
      }],
      allowed_skills: None,
      ...
      auto_inject_max_items: 20,
    }
  }

  #[test]
  fn discovers_and_loads_skill_md() {
    let root = test_dir("discover");
    write_skill(&root, "writer", "writer", "Writing guidance");
    let config = config(root);
    let registry = SkillRegistry::discover(&config).expect("skills should discover");
    let descriptors = registry.descriptors(None).expect("descriptors should load");
    let loaded = registry
      .load("writer", Some("draft".to_string()), &config)
      .expect("skill should load");

    assert_eq!(descriptors.len(), 1);
    assert_eq!(descriptors[0].description.as_deref(), Some("Writing guidance"));
    assert_eq!(descriptors[0].status, 1);
    assert_eq!(descriptors[0].status_label, "active");
    assert_eq!(loaded.args.as_deref(), Some("draft"));
    assert!(loaded.prompt.contains("Use this skill carefully."));
    assert!(!loaded.prompt.contains("description:"));
  }

  #[test]
  fn context_injection_lists_active_skills() {
    let root = test_dir("inject");
    write_skill(&root, "writer", "writer", "Writing guidance");
    let config = config(root);
    let injection = super::build_context_injection(&config)
      .expect("injection should build")
      .expect("injection should exist");

    assert!(injection.contains("Available Yuanling skills"));
    assert!(injection.contains("writer"));
    assert!(injection.contains("Skill tool"));
  }

  #[test]
  fn allowed_skills_filter_injections() {
    let root = test_dir("allowed");
    write_skill(&root, "writer", "writer", "Writing guidance");
    write_skill(&root, "reviewer", "reviewer", "Review guidance");
    let mut config = config(root);
    config.allowed_skills = Some(BTreeSet::from(["writer".to_string()]));
    let registry = SkillRegistry::discover(&config).expect("skills should discover");
    let injections = registry
      .injections(config.allowed_skills.as_ref())
      .expect("injections should load");

    assert_eq!(injections.len(), 1);
    assert_eq!(injections[0].id, "writer");
  }

  #[test]
  fn duplicate_skills_are_shadowed_by_first_root() {
    let first = test_dir("first");
    let second = test_dir("second");
    write_skill(&first, "plan", "plan", "First");
    write_skill(&second, "plan", "plan", "Second");
    let config = SkillsModuleConfig {
      enabled: true,
      roots: vec![
        SkillRoot {
          source: SkillSource::DataDir,
    ...
    assert_eq!(registry.descriptors(None).unwrap().len(), 1);
    assert_eq!(registry.skills.iter().filter(|record| !record.descriptor.enabled).count(), 1);
  }
  ...
  fn search_matches_name_and_description() {
    ...
    let config = config(root);
    let registry = SkillRegistry::discover(&config).expect("skills should discover");
    let output = registry.search("writing", 5);

    assert_eq!(output.matches.len(), 1);
    assert_eq!(output.matches[0].id, "writer");
  }

  #[test]
  fn install_skill_copies_skill_md_into_data_root() {
    let source = test_dir("install-source");
    let target = test_dir("install-target");
    write_skill(&source, "formatter", "formatter", "Format guidance");
    let config = config(target.clone());
    let installed = install_skill(&source.join("formatter"), &config).expect("skill should install");

    assert_eq!(installed.id, "formatter");
    assert!(target.join("formatter").join("SKILL.md").is_file());
  }

  #[test]
  fn disabled_config_rejects_load() {
    let root = test_dir("disabled");
    write_skill(&root, "writer", "writer", "Writing guidance");
    let mut config = config(root);
    config.enabled = false;
    let registry = SkillRegistry::discover(&config).expect("disabled discovery should be empty");
    let result = registry.load("writer", None, &config);

    assert!(matches!(result, Err(SkillError::Disabled)));
  }
}
